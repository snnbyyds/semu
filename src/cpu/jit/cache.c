...
uint8_t *jit_cache_add(cache_t *cache, vaddr_t pc, const void *code, size_t size, uint64_t align) {
...
bool jit_cache_hot(cache_t *cache, vaddr_t pc) {
    uint64_t idx = hash(pc, CACHE_ENTRY_SIZE);
    while (cache->table[idx].pc) {
        if (cache->table[idx].pc == pc) {
            cache->table[idx].hot++;
